...
const services = [
  { icon: Code2, title: 'Sites web professionnels', description: 'Site vitrine, portfolio, landing page ou plateforme métier responsive, rapide et optimisée pour le référencement.', price: '100 000 – 500 000 FCFA', tag: 'Web' },
  { icon: Users, title: 'Applications mobiles', description: 'Applications Android et iOS de tout type : éducation, commerce, communauté, productivité ou service interne.', price: '300 000 – 2 000 000 FCFA', tag: 'Mobile' },
  { icon: BookOpen, title: 'Applications web & SaaS', description: 'Interfaces modernes, espaces membres, tableaux de bord, workflows et plateformes collaboratives.', price: '250 000 – 1 500 000 FCFA', tag: 'Produit' },
  { icon: MessageCircle, title: 'API & back-end', description: 'API REST, authentification, bases de données, synchronisation offline-first, intégrations et déploiement.', price: '150 000 – 1 000 000 FCFA', tag: 'Technique' },
  { icon: Lightbulb, title: 'Logiciels desktop', description: 'Outils multiplateformes pour Windows, Linux et macOS, avec packaging, stockage local et documentation.', price: '300 000 – 1 800 000 FCFA', tag: 'Desktop' },
  { icon: Send, title: 'Linux & maintenance', description: 'Installation de distributions Linux, configuration poste, sauvegarde, nettoyage, mises à jour et assistance simple.', price: '10 000 – 150 000 FCFA', tag: 'Support' },
  { icon: UsersRound, title: 'UI/UX & identité produit', description: 'Parcours utilisateurs, wireframes, design system, maquettes et amélioration de l’expérience existante.', price: '50 000 – 400 000 FCFA', tag: 'Design' },
  { icon: Code2, title: '3D & expériences interactives', description: 'Modélisation 3D, visualisation produit, scènes interactives, assets web et prototypes immersifs.', price: '150 000 – 1 500 000 FCFA', tag: 'Créatif' },
]

const packages = [
  { name: 'Essentiel', price: '50 000 – 150 000 FCFA', description: 'Pour lancer une présence numérique propre et crédible.', features: ['Audit et cadrage du besoin', 'Landing page ou site simple', 'Version mobile responsive', 'Mise en ligne et prise en main'], accent: false },
  { name: 'Professionnel', price: '150 000 – 450 000 FCFA', description: 'Pour un site ou outil métier complet prêt à évoluer.', features: ['Conception UI/UX', 'Site multi-pages ou API légère', 'SEO technique de base', 'Documentation et formation'], accent: true },
  { name: 'Application', price: '350 000 – 2 500 000 FCFA', description: 'Pour une application mobile ou une plateforme métier sur mesure.', features: ['Architecture et maquettes', 'Développement mobile ou web', 'Backend et base de données', 'Tests et accompagnement au lancement'], accent: false },
  { name: 'Sur mesure', price: '500 000 – 5 000 000+ FCFA', description: 'Pour les projets ambitieux, scolaires, associatifs ou institutionnels.', features: ['Périmètre défini ensemble', 'Équipe adaptée au projet', 'Sprints et livrables réguliers', 'Maintenance évolutive disponible'], accent: false },
]
...
export default function ServicesPage() {
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section id="tarifs" className="bg-[#f3eadf] px-4 py-20 md:py-28"><div className="container mx-auto max-w-6xl"><div className="mx-auto mb-12 max-w-2xl text-center"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#e95716]">Tarifs indicatifs</p><h2 className="mt-3 text-3xl font-black md:text-5xl">Choisissez un point de départ</h2><p className="mt-4 text-sm leading-6 text-[#65584f]">Ces plages donnent un ordre de grandeur en FCFA. Le périmètre, le délai, le niveau de finition et le budget influencent directement la qualité du travail, le temps de recherche, les tests, la documentation et l’accompagnement inclus. Le devis final est toujours défini après échange.</p></div><div className="grid gap-5 lg:grid-cols-4">{packages.map((pack) => <article key={pack.name} className={`rounded-[1.35rem] border p-7 ${pack.accent ? 'border-[#ff7626] bg-[#17120f] text-white shadow-[0_14px_0_#ff7626]' : 'border-[#eadfd4] bg-[#fffaf0] text-[#17120f]'}`}><h3 className={`text-xl font-black ${pack.accent ? 'text-[#ff9a5a]' : 'text-[#17120f]'}`}>{pack.name}</h3><p className={`mt-4 text-2xl font-black ${pack.accent ? 'text-white' : 'text-[#e95716]'}`}>{pack.price}</p><p className={`mt-4 min-h-[72px] text-sm leading-6 ${pack.accent ? 'text-white/70' : 'text-[#65584f]'}`}>{pack.description}</p><ul className={`mt-6 space-y-3 border-t pt-5 text-sm ${pack.accent ? 'border-white/15 text-white/75' : 'border-[#eadfd4] text-[#65584f]'}`}>{pack.features.map((feature) => <li key={feature} className="flex gap-2"><span className="font-black text-[#56b88b]">✓</span>{feature}</li>)}</ul><a href="/contact" className={`mt-7 inline-flex items-center gap-2 text-sm font-black ${pack.accent ? 'text-[#ff9a5a]' : 'text-[#e95716]'}`}>Demander un devis <ArrowUpRight className="h-4 w-4" /></a></article>)}</div></div></section>
