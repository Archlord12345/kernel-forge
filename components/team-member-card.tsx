...
export function TeamMemberCard({ member }: TeamMemberCardProps) {
  ...
  return (
    <div className="group text-center p-6 rounded-xl bg-card border border-border hover:border-primary transition-all hover:shadow-lg hover:-translate-y-1 duration-300">
      {/* Avatar */}
      <div className="mb-4 inline-block">
        {profile?.avatar_url ? (
          <img
            ...
            alt={name}
            className="w-24 h-24 rounded-lg object-cover border-2 border-primary/20 group-hover:border-primary transition-colors"
          />
        ) : (
          <div className="w-24 h-24 rounded-lg bg-primary/20 border-2 border-primary/20 group-hover:border-primary transition-colors flex items-center justify-center font-bold text-primary text-lg">
            {initials}
          </div>
        )}
      </div>
